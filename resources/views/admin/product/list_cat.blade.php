...
@section('content')
    ...
    @endif
        ...
        @php
            function TableProductCat($product_cats, $parent_id = 0, $char = '')
            {
                $t = 0;
                foreach ($product_cats as $key => $item) {
                    ...
                    if ($item->parent_id == $parent_id) {
                        ...
                        echo '<td>' . $char . $item->slug . '</td>';
                        echo '<td>' . $item->parent_id . '</td>';
                        echo '<td>' . $char . $item->cat_id . '</td>';
                        echo '<td>' . $item->created_at->format('d,m,y') . '</td>';
                        echo '<td>';
                        echo '<a href="' .
                            route('product_cat.edit', $item->id) .
                            '" class="btn btn-success btn-sm rounded-0 text-white" type="button"
                                                    data-toggle="tooltip" data-placement="top" title="Edit"><i
                                                    class="fa fa-edit"></i></a>
                                            <a href="' .
                            route('delete_product_cat', $item->id) .
                            '"
                        ...
                        TableProductCat($product_cats, $item->id, $char . '--');
                    }
                }
            }
        @endphp
